			   ...
			   u8  reg, u8  mask, u8  set)
{
	...
	dev_dbg(&client->dev, "masks: 0x%02x, 0x%02x", reg, val);
			     ...
			     const struct ov2640_win_size *win, u32 code)
{
	...
	/* select preamble */
	dev_err(&client->dev, "%s: Set size to %s", __func__, win->name);
	...
	/* cfmt preamble */
	dev_err(&client->dev, "%s: Set cfmt", __func__);
...
static int ov2640_video_probe(struct i2c_client *client)
{
	...
	dev_err(&client->dev, "%s: Init default", __func__);
			...
			const struct i2c_device_id *did)
{
	...
	priv->sensor_clk = devm_clk_get(dev, "csi_mclk");
	if (IS_ERR(priv->sensor_clk)) {
		dev_err(dev, "get mclk failed\n");
		return PTR_ERR(priv->sensor_clk);
	}
	...
	ret = clk_set_rate(priv->sensor_clk, priv->mclk);
	if (ret < 0)
		return ret;

	clk_prepare_enable(priv->sensor_clk);
	...
	dev_err(&adapter->dev, "OV2640 Probed\n");
...
MODULE_AUTHOR("Embedfire Semiconductor, Inc.");
