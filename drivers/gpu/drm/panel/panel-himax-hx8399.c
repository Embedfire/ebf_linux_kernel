...
struct hx8399_panel_variant {
	void (*init_sequence)(struct hx8399 *ctx);
};
...
struct hx8399 {
	...
	bool prepared;
	bool enabled;
	const struct hx8399_panel_variant *variant;
};
...
static void hx8399_original_init(struct hx8399 *ctx)
{
...
static void hx8399_v1_init(struct hx8399 *ctx)
{
	dcs_write_seq(ctx, 0xb0, 0x04);
	dcs_write_seq(ctx, 0xd6, 0x01);
	dcs_write_seq(ctx, 0xB3, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00);
	dcs_write_seq(ctx, 0xB4, 0x0C, 0x00);
	dcs_write_seq(ctx, 0xB6, 0x4B, 0xDB, 0x00);
	dcs_write_seq(ctx, 0xC1, 0x04, 0x60, 0x00, 0x3F, 0x98, 0xF3, 0xC7, 0x2F, 0xFF, 0xFF, 0xFF,0x5D, 0x63, 0xAC, 0xB9, 0xFF, 0xFF, 0xFF, 0xE4, 0x8B, 0x7F, 0x0C, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x6B, 0x11, 0x02, 0x21, 0x00, 0x01, 0x11);
	dcs_write_seq(ctx, 0xC2, 0x31, 0xF7, 0x80, 0x08, 0x08, 0x00, 0x00, 0x08);
	dcs_write_seq(ctx, 0xC4, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x06);
	dcs_write_seq(ctx, 0xC6, 0xC8, 0x01, 0x76, 0x06, 0x6A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x1C, 0x07, 0xC8);
	dcs_write_seq(ctx, 0xC7, 0x02, 0x0E, 0x17, 0x22, 0x31, 0x41, 0x49, 0x58, 0x3C, 0x45, 0x52, 0x61, 0x6A, 0x71, 0x7C, 0x02, 0x0E, 0x17, 0x22, 0x31, 0x41, 0x49, 0x58, 0x3C, 0x45, 0x52, 0x61, 0x6A, 0x71, 0x7C);
	dcs_write_seq(ctx, 0xCB, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xE0, 0x00, 0x00);
	dcs_write_seq(ctx, 0xCC, 0x0E);
	dcs_write_seq(ctx, 0xD0, 0x11, 0x00, 0x00, 0x54, 0xCA, 0x40, 0x19, 0x19, 0x09, 0x00);
	dcs_write_seq(ctx, 0xD1, 0x04, 0x48, 0x06, 0x0D);
	dcs_write_seq(ctx, 0xD3, 0x1B, 0x33, 0xBB, 0xBB, 0xB3, 0x33, 0x33, 0x33, 0x33, 0x00, 0x01, 0x00, 0x00, 0xD8, 0xA0, 0x08, 0x4C, 0x4C, 0x33, 0x33, 0x72, 0x12, 0x8A, 0x57, 0x3D, 0xBC);
	dcs_write_seq(ctx, 0xD5, 0x06, 0x00, 0x00, 0x01, 0x2B, 0x01, 0x2B);
	dcs_write_seq(ctx, 0x3a, 0x70);
	dcs_write_seq(ctx, 0xde, 0x00);
...
static const struct of_device_id himax_hx8399_of_match[] = {
	{
		...
		.data = &(const struct hx8399_panel_variant){
			.init_sequence = hx8399_original_init,
		},
	},
	{
		.compatible = "himax,hx8399v1",
		.data = &(const struct hx8399_panel_variant){
			.init_sequence = hx8399_v1_init,
		},
	},
	{ }
};
...
static int hx8399_probe(struct mipi_dsi_device *dsi)
{
	...
	match = of_match_device(himax_hx8399_of_match, dev);
	if (!match) {
		dev_err(dev, "no matching panel variant found\n");
		return -ENODEV;
	}
	ctx->variant = match->data;
	...
	dev_info(&dsi->dev, "probed %s panel variant\n", match->compatible);
